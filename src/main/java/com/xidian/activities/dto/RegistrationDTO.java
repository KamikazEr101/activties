...
public class RegistrationDTO {
    ...
    @Schema(description = "学生姓名", example = "张三", requiredMode = RequiredMode.REQUIRED)
    private String studentName;

    @NotBlank(message = "手机号不能为空")
    @Pattern(regexp = "^1[3-9]\\d{9}$", message = "手机号格式不正确")
    @Schema(description = "学生手机号", example = "13800138000", requiredMode = RequiredMode.REQUIRED)
    private String studentPhone;

    @NotBlank(message = "学院不能为空")
    @Size(max = 100, message = "学院名称长度不能超过100字符")
    @Schema(description = "学生学院", example = "计算机学院", requiredMode = RequiredMode.REQUIRED)
